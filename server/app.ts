...
export function createApp(config: AppConfig = readConfig(), authRuntime: () => AuthService | null = getAuthRuntime) {
  ...
  app.get('/api/health', c => c.json(envelope(healthSchema).parse({
    data:{ status:'ok', phase:'P02', environment:config.environment, demoEnabled:config.demoEnabled && config.environment !== 'production' },
