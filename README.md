...
**운영 Supabase 업무·인증 DB의 연결, TLS 검증과 역할 분리를 확인했습니다. 메일·문자·소셜 앱 연결 및 실계정 왕복 검증은 남아 있습니다.** API 키가 없는 방법은 화면에 준비 중으로 표시합니다. 내부 테스트 통과를 외부 공급자 개통이나 P02 전체 완료로 간주하지 않습니다. 다섯 로그인 방법은 정식 출시 범위에 유지합니다.
...
| 설정 | 용도 |
| --- | --- |
| `AUTH_DATABASE_URL` | `auth_runtime` 권한만 가진 별도 비소유자 로그인 역할. 현재 운영은 `iharu_auth_login`; 인증 스키마만 접근 |
...
마이그레이션 관리자, `auth_runtime`·`app_runtime` 권한 역할, 작업 계정은 분리합니다. 새 환경에서는 필요한 권한 역할을 먼저 만들고 관리자 연결로 아래 마이그레이션을 적용합니다. 로그인은 별도의 비소유자 계정에 해당 권한 역할 하나만 부여합니다. 현재 운영은 `iharu_auth_login` → `auth_runtime`, `iharu_app_login` → `app_runtime`이며 두 계정에 서로의 역할이나 `iharu_policy`를 부여하지 않습니다. 비밀번호는 환경별로 별도 설정합니다.
