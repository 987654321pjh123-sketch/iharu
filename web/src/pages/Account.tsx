...
function VerifiedAccount({name,email,emailVerified}:{name:string;email?:string|null;emailVerified?:boolean}){
  ...
  return <><div className="account-heading"><div><span className="eyebrow">내 계정</span><h1>로그인 방법과 기기</h1><p>{name}님의 계정을 한곳에서 관리해요.</p></div><span className="account-symbol"><Icon name="shield"/></span></div>
    ...
    <section className="account-overview"><span className="badge green">계정 로그인 완료</span><p>{email||'연락받을 이메일을 연결해 주세요.'}{email&&!emailVerified&&' · 이메일 확인 필요'}</p><small>아이 정보는 보호자 확인·동의·가족 연결 후 사용할 수 있어요. 가족 화면에서 연결 준비 상태를 확인해 주세요.</small><Link className="button primary full" to="/family">우리 가족 연결하기 <Icon name="arrow"/></Link></section>
