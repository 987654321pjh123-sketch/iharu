...
export function AuthLayout({ children, wide = false }: { children: ReactNode; wide?: boolean }) {
  ...
  useEffect(() => { document.title = '로그인과 계정 · 아이하루'; window.scrollTo(0,0); document.getElementById('auth-main')?.focus({preventScroll:true}); }, [pathname]);
  return <div className="auth-app"><a className="skip-link" href="#auth-main">본문 바로가기</a>
    ...
    </main><footer className="auth-footer">아이하루 · 가족의 하루를 잇다</footer>
  </div>;
}
